...
void visualize(int x=-1, int y=-1, int z=-1)
{
    ...
    for(int i=0; i<=SCREEN_WIDTH-rectSize; i+=rectSize)
    {
        ...
        {
            SDL_SetRenderDrawColor(renderer, 147, 112, 219, 0);
            SDL_RenderFillRect(renderer, &rect);
        }
        else if(j==y)
        {
            SDL_SetRenderDrawColor(renderer, 255, 248, 23, 0);
            SDL_RenderFillRect(renderer, &rect);
        }
        else
        {
            SDL_SetRenderDrawColor(renderer, 204, 0, 0, 0);
            SDL_RenderDrawRect(renderer, &rect);
        }
        j++;
    }
...
bool controls()
{
    cout <<"WARNING: Giving repetitive commands may cause latency and the visualizer may behave unexpectedly. Please give a new command only after the current command's execution is done.\n\n"
         ...
         <<"    Use 2 to start Merge Sort Algorithm.\n"
...
void intro()
{
    cout<<"------------------------------SORTING VISUALIZER------------------------------\n\n"
        <<"The program is set up to sort a list of 130 elements, which can be randomized and sorted using any of the available algorithms. All of the sorting algorithms sort the list in ascending order.\n"
        <<"The visualization of sorting times is not precisely the same as the actual time complexities of the algorithms. In order to provide a clearer visualization, the relatively faster algorithms like Merge Sort have been slowed down.\n\n"
;
    string s;
    getline(cin, s);
    if(s=="\n")
